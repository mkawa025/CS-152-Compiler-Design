...
{IDENT}	{yylval.ident = strdup(yytext); currPos += yyleng; return IDENT;}

{DIGITS}{DIGITS}*	{yylval.num = atoi(yytext); currPos += yyleng; return NUMBER;}
(\.{DIGITS}+)|({DIGITS}+(\.{DIGITS}*)?([eE][+-}?[0-9]+)?) {yylval.num = atoi(yytext); currPos += yyleng; return NUMBER;}
